...
function renderDaily() {
  ...
  const dayOrders = allOrders.filter(o => { const dt=o.createdAt?.toDate(); return dt&&dt>=start&&dt<=end&&o.status==='paid'; });
  ...
  renderStatusChart(allOrders.filter(o=>{ const dt=o.createdAt?.toDate(); return dt&&dt>=start&&dt<=end; }), 'dayStatusChart');
}
...
function renderHourlyChart(orders) {
  ...
  orders.forEach(o=>{ const d=o.createdAt?.toDate(); if(d){const h=d.getHours();if(h>=6&&h<=23)hours[h]+=(o.total||0);} });
  const maxVal = Math.max(...Object.values(hours),1);
  const hasAny = Object.values(hours).some(v=>v>0);
  if (!hasAny) { el.innerHTML='<div class="empty-detail"><div class="empty-detail-icon">🕐</div><div class="empty-detail-text">No sales recorded for this day</div></div>'; return; }
...
function renderWeekly() {
  ...
  const weekOrders = allOrders.filter(o=>{const dt=o.createdAt?.toDate();return dt&&dt>=ws&&dt<=weekEnd&&o.status==='paid';});
  ...
  renderStatusChart(allOrders.filter(o=>{const dt=o.createdAt?.toDate();return dt&&dt>=ws&&dt<=weekEnd;}),'weekStatusChart');
}
...
function renderWeeklyBars(days, weekOrders) {
  ...
  el.querySelectorAll('.weekly-bar-track').forEach(track=>{
    ...
    track.addEventListener('click',()=>{
      ...
      const detailEl=document.getElementById('weeklyDayDetail'),titleEl=document.getElementById('weeklyDayDetailTitle'),kpisEl=document.getElementById('weeklyDayKpis');
      ...
      const rev=dayOrd.reduce((s,o)=>s+(o.total||0),0),avg=dayOrd.length?rev/dayOrd.length:0;
      kpisEl.innerHTML=`
        <div class="day-kpi gold"><div class="day-kpi-label">Revenue</div><div class="day-kpi-value">${fmtCurrency(rev)}</div></div>
        <div class="day-kpi"><div class="day-kpi-label">Orders</div><div class="day-kpi-value">${dayOrd.length}</div></div>
        <div class="day-kpi"><div class="day-kpi-label">Avg. Order</div><div class="day-kpi-value">${dayOrd.length?fmtCompact(avg):'—'}</div></div>`;
...
function renderMonthly() {
  ...
  const [year,monthStr] = selectedMonth.split('-'); const month=parseInt(monthStr)-1;
  const startDate=new Date(year,month,1), endDate=new Date(year,month+1,0,23,59,59);
  const monthOrders=allOrders.filter(o=>{const d=o.createdAt?.toDate();return d&&d>=startDate&&d<=endDate&&o.status==='paid';});
  const monthRev=monthOrders.reduce((s,o)=>s+(o.total||0),0), avg=monthOrders.length?monthRev/monthOrders.length:0;
  ...
  renderStatusChart(allOrders.filter(o=>{const d=o.createdAt?.toDate();return d&&d>=startDate&&d<=endDate;}),'statusChart');
}
...
function renderCalendar(startDate, monthOrders) {
  ...
  const dailySales={};
  monthOrders.forEach(o=>{const d=o.createdAt?.toDate();if(d){const key=toDateKey(d);if(!dailySales[key])dailySales[key]={revenue:0,count:0};dailySales[key].revenue+=(o.total||0);dailySales[key].count++;}});
  ...
  const todayKey=toDateKey(today), todayCell=grid.querySelector(`[data-date="${todayKey}"]`);
  if(todayCell&&todayCell.classList.contains('has-sales')){todayCell.classList.add('selected');showDayDetails(todayKey,monthOrders);}
}
...
function showDayDetails(dateStr, monthOrders) {
  ...
  const itemCount={};
  dayOrders.forEach(o=>(o.items||[]).forEach(it=>{const k=it.name||'?';if(!itemCount[k])itemCount[k]={name:k,qty:0,amount:0};itemCount[k].qty+=(it.qty||1);itemCount[k].amount+=((it.price||0)*(it.qty||1));}));
  ...
  if(!sorted.length){tbody.innerHTML='<tr><td colspan="3"><div class="empty-detail"><div class="empty-detail-icon">🍽️</div><div class="empty-detail-text">No items recorded</div></div></td></tr>';return;}
  tbody.innerHTML=sorted.map(it=>`<tr><td style="font-weight:500">${it.name}</td><td style="color:var(--gold);font-weight:600;text-align:center">${it.qty}</td><td style="text-align:right;font-weight:600">${fmtCurrency(it.amount)}</td></tr>`).join('');
}

// ── Shared: top items ─────────────────────────────────────────────────────────
function renderTopItems(orders, containerId) {
  const itemCount={};
  orders.forEach(o=>(o.items||[]).forEach(it=>{const k=it.name||'?';if(!itemCount[k])itemCount[k]={name:k,category:it.category||'—',orders:0,revenue:0};itemCount[k].orders+=(it.qty||1);itemCount[k].revenue+=((it.price||0)*(it.qty||1));}));
  ...
  if(!sorted.length){container.innerHTML='<div class="empty-detail"><div class="empty-detail-icon">🍽️</div><div class="empty-detail-text">No data for this period</div></div>';return;}
...
function renderStatusChart(orders, chartId) {
  const statuses=['pending','preparing','served','paid','cancelled'], counts={};
  statuses.forEach(s=>counts[s]=0);
  orders.forEach(o=>{if(counts[o.status]!==undefined)counts[o.status]++;});
  const total=orders.length||1, max=Math.max(...Object.values(counts),1);
  const chartEl=document.getElementById(chartId); if(!chartEl) return;
  chartEl.innerHTML=statuses.map(s=>{
    const pct=Math.round((counts[s]/total)*100),barW=(counts[s]/max)*100;
    return `<div class="bar-row"><span class="bar-label">${capitalize(s)}</span><div class="bar-track"><div class="bar-fill ${s}" style="width:0%" data-target="${barW}%"></div></div><span class="bar-count">${counts[s]}</span><span class="bar-pct">${pct}%</span></div>`;
  }).join('');
