...
/* ─── Toast ─── */
let showToast = (m, type = 'default') => console.log(m);
...
/* ─── Confirmation Modal ─── */
function showConfirmModal({ title, message, icon, confirmLabel, confirmClass, onConfirm }) {
  ...
  confirmBtn.className = 'btn-sm ' + (confirmClass || 'gold');
  ...
  newBtn.className = 'btn-sm ' + (confirmClass || 'gold');
  newBtn.onclick = () => { close(); if (onConfirm) onConfirm(); };
...
function renderStaff(staff) {
  ...
  if (pending.length > 0) {
    html += `
      ...
      <div class="staff-section-label">⏳ Pending Approval</div>`;
    ...
    html += pending.map(s => `
      <div class="staff-card pending-card">
        ...
        <div class="staff-info">
          ...
          ${s.phone ? `<div class="staff-meta">${escapeHtml(s.phone)}</div>` : ''}
  ...
  if (staffBadge) {
    staffBadge.textContent = pending.length;
    staffBadge.style.display = pending.length > 0 ? 'inline-flex' : 'none';
  }
}
...
window._approveStaff = (uid, name) => {
  showConfirmModal({
    title: 'Approve Staff',
    message: `Grant ${name || 'this waiter'} access to the system? They will be able to log in and manage orders.`,
    icon: '✅',
    confirmLabel: '✓ Approve',
    confirmClass: 'gold',
    onConfirm: () => {
      updateDoc(doc(db, 'Users', uid), { status: 'approved', approvedAt: serverTimestamp() })
        .then(() => { showToast(`${name || 'Waiter'} has been approved.`, 'success'); loadStaff(); })
        .catch(e => { console.error(e); showToast('Failed to approve. Please try again.', 'error'); });
    }
  });
};
...
window._rejectStaff = (uid, name) => {
  showConfirmModal({
    title: 'Suspend / Reject Staff',
    message: `${name || 'This waiter'} will be suspended and will not be able to log in. You can re-approve them later.`,
    icon: '🚫',
    confirmLabel: 'Suspend',
    confirmClass: 'danger',
    onConfirm: () => {
      updateDoc(doc(db, 'Users', uid), { status: 'rejected', rejectedAt: serverTimestamp() })
        .then(() => { showToast(`${name || 'Waiter'} has been suspended.`, 'default'); loadStaff(); })
        .catch(e => { console.error(e); showToast('Failed to suspend. Please try again.', 'error'); });
    }
  });
};
